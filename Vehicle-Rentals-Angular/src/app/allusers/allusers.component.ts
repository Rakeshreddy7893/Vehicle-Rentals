...
export class AllusersComponent implements OnInit {
  ...
  constructor(private router: Router,private service: UserService, private toastr: ToastrService,private imageService: ImageService,private documentService: DocumentService) {
    this.email = localStorage.getItem("email");
    this.ownerId = localStorage.getItem("userid");
  ...
  getAllApprovedImages() {
    this.documentService.getAllApprovedDocImages().subscribe(
      (res: any[]) => {
        ...
        this.allDocumentsApprovedImages.forEach((image) => {
          image.documentContent = 'data:image/jpeg;base64,' + image.documentContent;
        });
      },
      (error) => {
        console.error('Error retrieving not approved images:', error);
    ...
    // Method to show approved images
    showApprovedImages() {
      this.showApproved = true;
      this.showUnapproved = false;
      this.getAllApprovedImages(); // Call method to fetch approved images
    }
  
    // Method to show unapproved images
    showUnapprovedImages() {
      this.showUnapproved = true;
      this.showApproved = false;
      this.getAllNotApprovedImages(); // Call method to fetch unapproved images
    }
  ...
  toggleDocumentStatus(image : any){
    ...
    const newStatus = this.presentDocumentStatus === 'Approved' ? 'Not Approved' : 'Approved';
    this.documentService.updateDocumentImageStatus(newStatus, image.documentId).subscribe(() => {
