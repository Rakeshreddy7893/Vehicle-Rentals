...
public class DocumentsUploadController {
    ...
    public boolean uploadCustomerDocuments(@RequestParam("file") MultipartFile file,
        ...
        try {
            ...
            documents.setStatus("Not Approved"); // Default status

            // Set the user
            documents.setUser(userRepository.findById(customerId));

            // Compress and set the image bytes
    ...
    public void updateDocument(@PathVariable("status") String status, @PathVariable("documentId") String documentId) throws IOException {
            ...
            if (doc != null) {
                ...
                if (documentContent != null) {
                	DocumentResponse response = new DocumentResponse(
                            ...
                            doc.getUser(),
                            doc.getStatus(),
                            decompressBytes(documentContent)
                    );
                    return ResponseEntity.ok(response);
                } else {
                    // Handle case where document content is null
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                         .body(new DocumentResponse());
                }
            } else {
                // Handle case where no record is found for the customer
